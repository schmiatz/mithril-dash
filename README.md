...
it purely from the outside (log tailing, Prometheus scraping, state-file
polling) — no changes to mithril, no code dependency on it, and no calls
into mithril's RPC server (deliberately: everything it could offer here was
either redundant with these sources or not worth the extra load on the node).
